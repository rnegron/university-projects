...
class mds_db:
    ...
    def AddDataNode(self, address, port):
        ...
        query = """insert into dnode (address, port) values ("%s", %s)""" % (
            address, port)
        try:
    ...
    def CheckNode(self, address, port):
        """Check if node is in database and returns name, address, port
           for connection.
        """
        query = """select nid from dnode where address="%s" and port=%s""" % (
            address, port)
        try:
            self.c.execute(query)
        except:
    ...
    def InsertFile(self, fname, fsize):
        """Create the inode attributes.  For this project the name of the
           file and its size.
        """
        query = """insert into inode (fname, fsize) values ("%s", %s)""" % (
            fname, fsize)
        try:
            self.c.execute(query)
            return 1
    ...
    def AddBlockToInode(self, fname, blocks):
        ...
        for address, port, chunkid in blocks:
            ...
            if nid:
                query = """insert into block (nid, fid, cid) values (%s, %s, "%s")""" % (
                    nid, fid, chunkid)
                self.c.execute(query)
            else:
                return 0
